...
; Add Tab Control
Gui, Add, Tab3, x10 y10 w765 h680, Crops Selections|Calculator|Credits

; === TAB 1: CALCULATOR ===
Gui, Tab, 1

; === LEFT PANEL: Crop Radio Buttons ===
Gui, Add, GroupBox, x20 y40 w740 h450, Select Crop:
startX := 30
...
Gui, Add, Text, x%mainUIStartX% y+10 w400 vFruitBaseText cGray
...
Gui, Add, GroupBox, x%mainUIStartX% y+30 w450 h260, Environmental Mutations (multi-select):
colCount := 3
colWidth := 150
rowHeight := 22
...
Gui, Add, Text, x%mainUIStartX% y+30, Weight (kg):
...
Gui, Add, Text, x+5 yp, Value to Weight:
Gui, Add, Edit, x%mainUIStartX% y+5 w100 vMass, 1
Gui, Add, Slider, x+5 yp w100 Range0-5 TickInterval1 vFriendBoost gFriendBoostChanged, 0
Gui, Add, Edit, x+20 yp w100 vPriceInput
Gui, Add, Button, gCalculate x%mainUIStartX% y+10 w100, Calculate Price
Gui, Add, Button, gReset x+10 yp w100, Reset
Gui, Add, Button, gCalcWeight x+15 yp, Calculate Weight

Gui, Add, Text, x%mainUIStartX% y+2 w222 cBlue vGuiOutputPrice,
Gui, Add, Text, vWeightResult w400 x+2 yp cBlue
Gui, Add, Text, x%mainUIStartX% y+2 w400 cGray vGuiOutputMultiplier,
Gui, Add, Text, x%mainUIStartX% y+2 w400 cGreen vGuiOutputValuationDetails,
Gui, Add, Text, x%mainUIStartX% y+2 w400 cFuchsia vGuiOutputOutputMutations,
...
; === TAB 3: CREDITS ===
Gui, Tab, 3
Gui, Add, Text, x20 y40 w900 h600, % "CREDITS`n`n"
    . "Created by: Komuna`n"
    . "Version: 1.0`n"
    . "Special thanks to:`n"
    ...
    . "`n"
    . "© 2025 Grow a Garden Price Calculator"

; End of tabs
Gui, Tab

; === Show GUI ===
Gui, Show, w785 h700, Grow a Garden Price Calculator
...
Calculate:
    ...
    FormatSuffix(number) {
        if (number >= 1000000000000)
            return Round(number/1000000000000, 2) " Trillion"
    ...
    ; Build output lines
    OutputTotalPrice := FormattedPrice
    OutputTotalMultiplier := "Total Multiplier: x" . TotalMultiplier
    OutputValuationDetails := selectedCrop . " weighing (" . Mass . " kg) , worth " . FormattedModifiedPrice . " Sheckles"
    ...
    GuiControl,, GuiOutputOutputMutations, %OutputMutationList%
return
